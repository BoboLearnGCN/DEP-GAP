...
#   Rscript unsupervised_K5_revised.R -o results
...
parse_args <- function(args) {
  ...
  while (i <= length(args)) {
    ...
    if (arg %in% c("-h", "--help")) {
      cat(
        "Usage: Rscript unsupervised_K5_revised.R -o OUTPUT_DIR\n\n",
        "Options:\n",
...
write.csv(alpha_post, output_file("alpha_K5.csv"))
write.csv(mu_post, output_file("mu_K5.csv"))
write.csv(rocktype_post, output_file("rocktype_K5.csv"))
write.csv(gamma_post, output_file("gamma_K5.csv"))
write.csv(sigma_sq_post, output_file("sigma_sq_K5.csv"))
...
for (b in seq_len(B)) {
  ...
  ggsave(
    output_file(paste0("Clustering_K5_", b, ".png")),
...
if (nrow(psi_record) != n_psi) {
  stop("psi.txt must contain 10 rows for K=5; found ", nrow(psi_record), ".", call. = FALSE)
}

psi_index <- c(
  paste0("psi_1", 2:5), paste0("psi_2", 3:5), paste0("psi_3", 4:5), "psi_45")
...
write.csv(
  data.frame(K = K, criterion = "independent_mixture_plugin_BIC", BIC = BIC_K5),
  output_file("BIC_K5.csv"), row.names = FALSE
)
